...
public class MazeConfig {
    ...
    public static MazeConfig makeExample1() {
        File maze = new File("src/main/resources/maze.txt"); // Création d'un objet File qui contient le fichier
                                                             // maze.txt
        iniDictionary();
        try {
            ...
            while (scanner.hasNextLine()) {
                ...
                for (int j = 0; j < line.length(); j++) {
                    ...
                    if (itemDictionary.containsKey(currentChar)) { // Si le caractère courant est présent dans le
                                                                   // dictionnaire
                        Cell currentItem = itemDictionary.get(currentChar); // On récupère la cellule correspondante
                        Content nextItem = (nextChar == 'e') ? ENERGIZER : (nextChar == 'd') ? DOT : NOTHING; // On
                                                                                                              // récupère
                                                                                                              // le
                                                                                                              // contenu
                                                                                                              // de la
                                                                                                              ...
                                                                                                              // caractère
                                                                                                              // suivant
                        grid[i][ji] = currentItem.updateNextItemType(nextItem); // On met à jour le contenu de la
                                                                                // cellule
